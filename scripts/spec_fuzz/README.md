...
Python 3 and Node are needed. The renderer is commonmark.js, pinned to the spec version the check is written against:

```sh
cd scripts/spec_fuzz && npm install
```

## Running

Random documents are written out, rendered by commonmark.js into the shape of the spec's examples, and read by the spec check the same way the spec is. `target` is a good place for the files, since git ignores it.

```sh
mkdir -p target/spec_fuzz
python scripts/spec_fuzz/generate.py 1 20000 target/spec_fuzz/inputs.json
node scripts/spec_fuzz/render.js target/spec_fuzz/inputs.json target/spec_fuzz/cases.json
...
python scripts/spec_fuzz/classify.py target/spec_fuzz/report.txt parse
python scripts/spec_fuzz/classify.py target/spec_fuzz/report.txt format
```

The first argument of `generate.py` is the seed, so a run can be repeated. Pass `inline` as its fourth argument for documents that are one paragraph of emphasis and links each, which is what exercises how a decoration's delimiters are chosen.
...
`classify.py` groups the failures by the first line (for the parser) or the first tag (for the formatter) that differs, and prints one example of each group. The groups that are known and not worth reading have names instead:
